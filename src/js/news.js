(function () {
    ...
    function initCalendarFilter(listRoot, newsItems = []) {
        const calendarBody = document.getElementById('calendario-corpo');
        const monthLabel = document.getElementById('mes-atual');
        const prevBtn = document.getElementById('prev-mes');
        const nextBtn = document.getElementById('next-mes');
        const groups = Array.from(listRoot.querySelectorAll('.grupo-noticias'));
        const emptyMessage = listRoot.querySelector('.mensagem-sem-resultados');

        if (!calendarBody || !monthLabel || !groups.length) {
            return;
        }

        // Configuração automática: Começa no mês atual e vai até o dia de hoje
        const hoje = new Date();
        let displayDate = new Date(hoje.getFullYear(), hoje.getMonth(), 1); 
        const minDate = new Date(2026, 1, 1);   // Limite inferior: 1 de Fevereiro de 2026
        const maxDate = hoje;                   // Limite superior: Dia atual
        ...
        function renderCalendar() {
            ...
            // Nome do mês
            const monthName = new Intl.DateTimeFormat('pt-BR', { month: 'long', year: 'numeric' }).format(displayDate);
            monthLabel.textContent = monthName;

            // Lógica dos botões
            prevBtn.disabled = displayDate <= minDate;
            nextBtn.disabled = displayDate >= new Date(maxDate.getFullYear(), maxDate.getMonth(), 1);
            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (let j = 0; j < 7; j++) {
                    ...
                    if (i === 0 && j < firstDay) {
                        ...
                        const fullDateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(currentDay).padStart(2, '0')}`;
                        
                        cell.textContent = currentDay;
                        
                        // Verificar se tem notícia
                        const hasNews = newsItems.some(item => item.publishedAt === fullDateStr);
                        if (hasNews) {
                            cell.classList.add('dia-com-noticia');
                            cell.title = 'Possui notícias';
                        }

                        cell.addEventListener('click', () => {
                            // Limpar ativos
                            calendarBody.querySelectorAll('td').forEach(c => c.classList.remove('dia-ativo'));
                            cell.classList.add('dia-ativo');

                            // Filtrar notícias
                            let totalVisible = 0;
                            groups.forEach(group => {
                                const groupDate = group.getAttribute('data-news-group');
                                // Comparar label formatado (ex: "17 de abril de 2026")
                                const formattedSelected = new Date(fullDateStr + 'T00:00:00').toLocaleDateString('pt-BR', {
                                    day: 'numeric', month: 'long', year: 'numeric'
                                });

                                const matches = groupDate === formattedSelected;
                                group.style.display = matches ? 'block' : 'none';
                                if (matches) {
                                    totalVisible++;
                                    // Garantir que todos os itens do grupo apareçam
                                    group.querySelectorAll('.item-noticia').forEach(item => item.style.display = 'block');
                                }
                            });
                            ...
                            if (emptyMessage) {
                                ...
                                if (totalVisible === 0) {
                                    emptyMessage.textContent = `Nenhuma notícia encontrada para o dia ${formattedSelected}.`;
                                }
                            }
                        });
                        date++;
    ...
    async function initNewsListPage() {
        ...
        try {
            ...
            initCalendarFilter(listContainer, news);
